// Using an Arduino Uno and a PCA9685 servo driver board, control the arm with keyboard inputs.
// No smooth motion
#include <Arduino.h>
#include <Wire.h>
...
const int INNER_HUB_RADIUS = 45, INNER_HUB_HEIGHT = 27, SERVO_MIN = 150, SERVO_MAX = 600, SERVO_STOP = 80, change = 15, handThetaChange = 10, yawChange = 10; // Initialize int constants
...
bool allowPrintStatus = false, busy = false;
...
void setup() {
  ...
  pwm.setPWM(yawServo, 0, angleToPulse(90));
  ...
  if (Serial.available() > 0 && !busy) {
    ...
    switch (direction) {
      ...
      case 'r': // Reset to home position
        goToPosition(150,100,90,0,true);
        break;
      case '0': // Set to known positions to attach components
        yaw_angle = 90;
        pwm.setPWM(yawServo, 0, angleToPulse(yaw_angle));
        pwm.setPWM(servo1, 0, angleToPulse(0));
        pwm.setPWM(servo2, 0, angleToPulse(0));
        pwm.setPWM(servo3, 0, angleToPulse(90));
        pwm.setPWM(gripperServo, 0, angleToPulse(SERVO_STOP));
        printStatus();
        break;
      case '1':
        goToPosition(275,-40,140,0);
        break;
      case '2':
        goToPosition(155,-20,90,-100);
        break;
      case '3':
        goToPosition(295,-20,80,-30);
        break;
      case '4':
        goToPosition(150,100,90,0,true); // Home
        delay(movementDelay);

        goToPosition(275,-40,140,0); // Position 1
        delay(movementDelay);
        ...
        goToPosition(155,-20,90,-100); // Position 2
        delay(movementDelay);
        ...
        goToPosition(295,-20,80,-30); // Position 3
        delay(movementDelay);
...
void smoothMotion(float start, float end, int servo) {
  int steps = 150, duration = abs(end - start) * 10;
...
void smoothMotionForTheta(float start1, float end1, float start2, float end2, float start3, float end3) {
  int steps = 150, duration = max(max(abs(end1 - start1), abs(end2 - start2)), abs(end3 - start3)) * 6;
  for (int i = 0; i <= steps; i++) {
    if (invalid == 1) {
      return;
    }
